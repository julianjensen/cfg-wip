 ...
 * @return {?string}
 */
function from_object_pattern( node )
{
    if ( node.type === Syntax.MemberExpression ) return null;

    if ( node.type === Syntax.Identifier )
        return node.name;
    else if ( node.type === Syntax.AssignmentPattern )
        return from_object_pattern( node.key );
    else if ( node.type === Syntax.ObjectPattern )
        return node.properties.map( from_object_pattern );
    else if ( node.type === Syntax.Property && node.kind === 'init' )
        return from_object_pattern( node.value );
...
class CFGBuilder
{
    ...
    constructor( { ast, scopes, collector, blockList } )
    {
        ...
        const topInfo = CFGBuilder.get_start_nodes( this.root );
    ...
    define( name, block )
    {
        this.scopes.current.add( name + ' [def]', block );
    }
    ...
    declare( name, block )
    {
        this.scopes.current.add( name + ' [decl]', block );
    }
    ...
    BreakStatement( node, current )
    {
        ...
        if ( s && ( !l || s.depth > l.depth ) )
            current.add_succs( this.switchExits[ this.switchExits.length - 1 ] );
        else if ( l && ( !s || l.depth > s.depth ) )
            current.add_succs( ...l.blocks[ l.blocks.length - 1 ].succs );
        else
    ...
    {
        this.add( node, current );
        this.scopes.function_scope( 'catch', node.param, node );
